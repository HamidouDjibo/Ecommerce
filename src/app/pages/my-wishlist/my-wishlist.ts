...
@Component({
  ...
  @if (store.wishlistCount() > 0){
    <div class="flex justify-between items-center mb-6">
      ...
      <span class="text-gray-500 text-xl">{{store.wishlistCount()}} produit(s)</span>
    </div>

    <div class="responsive-grid">
      @for (product of store.wishlistItems(); track product.id) {
        <app-product-card [product]="product">
          <button class="!absolute top-3 right-3 z-10 !bg-white border-0 shadow-md flex items-center justify-center cursor-pointer transition-all duration-200 hover:scale-110 hover:shadow-lg"
            matIconButton
